...
pub fn proto_packet_to_packet(p: &ProtoPacket) -> Packet {
    ...
    let mut my_meta: Meta =  Meta::default();

    if let Some(meta) = &p.meta {
        my_meta.size = meta.size as usize;
        my_meta.addr = meta.addr.parse().unwrap_or(UNKNOWN_IP);
        my_meta.port = meta.port as u16;
        if let Some(flags) = &meta.flags {
            if flags.simple_vote_tx {
                my_meta.flags.insert(PacketFlags::SIMPLE_VOTE_TX);
            }
            if flags.forwarded {
                my_meta.flags.insert(PacketFlags::FORWARDED);
            }
            if flags.tracer_packet {
                my_meta.flags.insert(PacketFlags::TRACER_PACKET);
            }
            if flags.repair {
                my_meta.flags.insert(PacketFlags::REPAIR);
            }
        }
        packet = Packet::new(data, my_meta);
    ...
    packet_batch.packets.into_iter().map(|proto_packet| {
        ...
        if let Some(meta) = &proto_packet.meta {
            ...
            if let Some(flags) = &meta.flags {
                ...
                if flags.discard {
                    my_meta.flags.insert(PacketFlags::DISCARD);
                }
            }
            let mut packet = Packet::new([0; PACKET_DATA_SIZE], my_meta);
        }
